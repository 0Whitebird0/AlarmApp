...
import com.example.shsfirstapp.databinding.ActivityMainBinding


class MainActivity : AppCompatActivity() {
    val viewModel: AlarmViewModel by lazy {
        val dbHelper = AlarmDbHelper.getInstance(this)
        val factory = AlarmViewModelFactory(application, dbHelper) // application 전달
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        appBarConfiguration = AppBarConfiguration(
            setOf(
                R.id.nav_home, R.id.nav_alarm, R.id.nav_mp3
            ), drawerLayout
        )
